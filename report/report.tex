...
%----------------------------------------
\subsection{Comparison with Na\"{a}ve Index Builder}
...
the number of keywords n, the number of document m and the length of
document since we have to check in each document for each
...
and not for each document. 

Note: In the state machine we do as much accesses to the database as
the number of document (in the worst case meaning we find at least one
keyword per document). 

Let’s compare the results for this algorithm with our system results
for different number of keywords and posts. 
...
theory. Our system is faster than the naive indexer whatever
parameters we are changing. In particular when the number of document
is becoming really important, the State machine approach prove to be
very efficient. Moreover, as soon as we are dealing with a number of
keywords that can be representative of industrial needs, the state
machine prove also to be far faster than the naive indexer.

Future work on this strategy would be to compare this approach with
less naive strategies to build an index. Moreover, as said in the
introduction, it would probably be interesting to add compression
methods to our algorithm in order to reduce the index size in the
database.
